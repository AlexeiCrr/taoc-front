...
export default function AdminDataGrid() {
  ...
  const columns = useMemo<ColumnDef<AdminResponse>[]>(() => {
            ...
            <TooltipProvider delayDuration={300}>
              <Tooltip>
                <TooltipTrigger asChild>
                  <span className="block max-w-[280px] truncate cursor-default">
                    {email}
                  </span>
                </TooltipTrigger>
                <TooltipContent>
    ...
    const frequencyColumns: ColumnDef<AdminResponse>[] = frequencyNames.map((frequencyName) => ({
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <Button onClick={handlePreviousPage} disabled={pagination.page === 1} variant="outline">Previous</Button>
          <Button onClick={handleNextPage} disabled={pagination.page >= pagination.totalPages} variant="outline">Next</Button>
        </div>
      </div>
    </div>
  )
